...
function buy() {
    bout.play();
    var scraps = (GET_FILE("Score"));
    if (scraps >= price[skin_name.indexOf(document.getElementById("skin_name").innerHTML).toString()]) {
        ...
        if (response) {
            ...
            SAVE_LINE("Score", scraps- price[skin_name.indexOf(document.getElementById("skin_name").innerHTML).toString()]);
            SAVE_LINE("Skins", array);
            document.getElementById("scraps").innerHTML = scraps- price[skin_name.indexOf(document.getElementById("skin_name").innerHTML).toString()];
            equip();
        }
    }
}
...
if (localStorage.getItem("TT_Data") != null) {
    ...
    for (let i = 0; i <= 85; i++){
        ...
        if (i.toString() == equiped_skin) {
            ima.className = "equippedSkin";
        } else {
            if (saved_skins.includes(i)) {
